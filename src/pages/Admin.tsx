...
const Admin = () => {
  ...
  const loadConfigs = async () => {
    const [printer, layout] = await Promise.all([
  ...
  const loadData = async () => {
    const [tickets, logs, pending] = await Promise.all([
      getTodayTickets(),
      getPrintLogs(),
      getPendingPrints(),
    ]);
    ...
    setPrintLogs(logs || []);
    setPendingPrints(pending || []);
  };

  const handleSavePrinter = async () => {
    setSaving(true);
    try {
      await updateSystemConfig("printer", printerConfig as any);
  ...
  const handleSaveLayout = async () => {
    setSaving(true);
    try {
      await updateSystemConfig("ticket_layout", layoutConfig as any);
      toast.success("Layout do ticket salvo!");
    } catch {
      toast.error("Erro ao salvar");
    } finally {
      setSaving(false);
  ...
  const handleTestPrint = async (method: "browser" | "print_server" | "cloud") => {
    const mockTicket = {
      id: "test",
      display_number: "N0001",
      ticket_type: "normal" as const,
      ticket_number: "test",
      status: "waiting" as const,
      patient_name: "Teste de Impressão",
    ...
    if (success) toast.success("Teste de impressão enviado!");
    else toast.error("Falha no teste de impressão");
  };

  const handleReprocessPending = async () => {
    toast.info("Reprocessando impressões pendentes...");
    for (const p of pendingPrints) {
      if (p.tickets) {
        await printViaBrowser(p.tickets);
      }
    }
    loadData();
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto p-6">
        <Tabs defaultValue="printer" className="space-y-6">
          <TabsList className="grid w-full grid-cols-4">
            <TabsTrigger value="printer" className="gap-2">
              <Printer className="h-4 w-4" /> Impressora
            </TabsTrigger>
            <TabsTrigger value="layout" className="gap-2">
              <LayoutTemplate className="h-4 w-4" /> Layout Ticket
            </TabsTrigger>
            <TabsTrigger value="history" className="gap-2">
              <History className="h-4 w-4" /> Histórico
            </TabsTrigger>
            <TabsTrigger value="logs" className="gap-2">
              <FileText className="h-4 w-4" /> Logs Impressão
            </TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="printer">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <CardContent className="space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    <Switch
                      ...
                      onCheckedChange={(v) => setPrinterConfig({ ...printerConfig, enabled: v })}
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <Select
                      ...
                      onValueChange={(v: any) => setPrinterConfig({ ...printerConfig, connectionType: v })}
                    >
                  ...
                  {printerConfig.connectionType === "usb" && (
                    <div className="space-y-3">
                      <div>
                        ...
                        <Input
                          value={printerConfig.usbVendorId}
                          onChange={(e) => setPrinterConfig({ ...printerConfig, usbVendorId: e.target.value })}
                        />
                      </div>
                      <div>
                        ...
                        <Input
                          ...
                          onChange={(e) => setPrinterConfig({ ...printerConfig, usbProductId: e.target.value })}
                        />
                      </div>
                    </div>
                  )}
                  ...
                  {printerConfig.connectionType === "serial" && (
                    <div className="space-y-3">
                      <div>
                        ...
                        <Input
                          value={printerConfig.serialPort}
                          onChange={(e) => setPrinterConfig({ ...printerConfig, serialPort: e.target.value })}
                        />
                      </div>
                      <div>
                        ...
                        <Input
                          ...
                          onChange={(e) => setPrinterConfig({ ...printerConfig, serialBaudrate: Number(e.target.value) })}
                        />
                      </div>
                    </div>
                  )}
                  ...
                  <div>
                    ...
                    <Select
                      ...
                      onValueChange={(v: any) => setPrinterConfig({ ...printerConfig, paperSize: v })}
                    >
                      <SelectTrigger><SelectValue /></SelectTrigger>
                      <SelectContent>
                  ...
                  <div>
                    ...
                    <Select
                      value={printerConfig.printMode}
                      onValueChange={(v: any) => setPrinterConfig({ ...printerConfig, printMode: v })}
                    >
                      <SelectTrigger><SelectValue /></SelectTrigger>
                      <SelectContent>
                        <SelectItem value="fast">Rápido (mínimo)</SelectItem>
                        <SelectItem value="detailed">Detalhado (completo)</SelectItem>
                      </SelectContent>
                    </Select>
                  </div>

                  <Button onClick={handleSavePrinter} disabled={saving} className="w-full">
              ...
              <Card>
                ...
                <CardContent className="space-y-4">
                  {[
                    { label: "Corte automático", key: "autoCut" },
                    { label: "Imprimir QR Code", key: "printQrCode" },
                    { label: "Imprimir logo da clínica", key: "printLogo" },
                    { label: "Imprimir CPF", key: "printCpf" },
                    { label: "Imprimir nome", key: "printName" },
                  ].map(({ label, key }) => (
                    <div key={key} className="flex items-center justify-between">
                      <Label>{label}</Label>
                      <Switch
                        checked={(printerConfig as any)[key]}
                  ...
                  <div className="border-t border-border pt-4 space-y-3">
                    ...
                    <div className="grid grid-cols-1 gap-2">
                      <Button onClick={() => handleTestPrint("browser")} variant="outline">
                        <TestTube className="h-4 w-4 mr-2" /> Via Navegador
                      </Button>
                      <Button onClick={() => handleTestPrint("print_server")} variant="outline">
                        <TestTube className="h-4 w-4 mr-2" /> Via Print Server
                      </Button>
                      <Button onClick={() => handleTestPrint("cloud")} variant="outline">
                        <TestTube className="h-4 w-4 mr-2" /> Via Cloud
                      </Button>
                    </div>
                  </div>
                  ...
                  {pendingPrints.length > 0 && (
                    <div className="border-t border-border pt-4">
                      <Button onClick={handleReprocessPending} variant="outline" className="w-full">
                        <RefreshCw className="h-4 w-4 mr-2" />
                        Reprocessar {pendingPrints.length} Pendente(s)
                      </Button>
                    </div>
                  )}
                </CardContent>
                ...
                <CardContent className="space-y-4">
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) => setLayoutConfig({ ...layoutConfig, clinicName: e.target.value })}
                    />
                  </div>
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) => setLayoutConfig({ ...layoutConfig, header: e.target.value })}
                    />
                  </div>
                  <div>
                    <Label>Rodapé</Label>
                    <Input
                      ...
                      onChange={(e) => setLayoutConfig({ ...layoutConfig, footer: e.target.value })}
                    />
                  </div>
                  <div>
                    <Label>Mensagem Personalizada</Label>
                    <Textarea
                      value={layoutConfig.customMessage}
                      onChange={(e) => setLayoutConfig({ ...layoutConfig, customMessage: e.target.value })}
                    />
                  </div>
                  <div>
                    <Label>Aviso LGPD</Label>
                    <Textarea
                      value={layoutConfig.lgpdNotice}
                      onChange={(e) => setLayoutConfig({ ...layoutConfig, lgpdNotice: e.target.value })}
                    />
                  </div>
                  <div className="flex items-center justify-between">
                    <Label>Exibir data e hora</Label>
                    <Switch
                      checked={layoutConfig.showDateTime}
                      onCheckedChange={(v) => setLayoutConfig({ ...layoutConfig, showDateTime: v })}
                    />
                  </div>
                  <div>
                    ...
                    <Select
                      ...
                      onValueChange={(v: any) => setLayoutConfig({ ...layoutConfig, fontSize: v })}
                    >
                      <SelectTrigger><SelectValue /></SelectTrigger>
                      <SelectContent>
                  ...
                  <div>
                    ...
                    <Select
                      ...
                      onValueChange={(v: any) => setLayoutConfig({ ...layoutConfig, alignment: v })}
                    >
                      <SelectTrigger><SelectValue /></SelectTrigger>
                      <SelectContent>
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      max="3"
                      step="0.1"
                      value={layoutConfig.lineSpacing}
                      onChange={(e) => setLayoutConfig({ ...layoutConfig, lineSpacing: Number(e.target.value) })}
                    />
                  </div>
                  <Button onClick={handleSaveLayout} disabled={saving} className="w-full">
                    <Save className="h-4 w-4 mr-2" />
                    Salvar Layout
                  </Button>
                </CardContent>
          ...
          <TabsContent value="history">
            <Card>
              <CardHeader>
                <CardTitle>Senhas de Hoje ({todayTickets.length})</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-border">
                        ...
                        <th className="text-left p-2">Guichê</th>
                  ...
                  {todayTickets.length === 0 && (
                    <p className="text-center py-8 text-muted-foreground">Nenhuma senha hoje</p>
                  )}
                </div>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent>
                <div className="overflow-x-auto">
                  ...
                  {printLogs.length === 0 && (
                    <p className="text-center py-8 text-muted-foreground">Nenhum log</p>
                  )}
                </div>
              </CardContent>
            </Card>
...
function TicketPreview({ layout, config }: { layout: TicketLayout; config: PrintConfig }) {
  ...
  return (
    <div className={`font-mono space-y-1 ${alignClass}`} style={{ lineHeight: layout.lineSpacing + 0.4 }}>
      ...
      <p className={`${fontSizeMap[layout.fontSize]} font-black text-primary tracking-wider`}>
        N0001
      </p>
      <p className="text-xs">Tipo: Normal</p>
      {layout.showDateTime && (
        <p className="text-xs">Data: {new Date().toLocaleString("pt-BR")}</p>
      )}
      ...
      {config.printQrCode && (
        <div className="flex justify-center my-2">
          <div className="w-16 h-16 border-2 border-border flex items-center justify-center text-xs text-muted-foreground">
            QR Code
          </div>
        </div>
      )}
      ...
      {layout.lgpdNotice && (
        <p className="text-[8px] text-muted-foreground mt-1">{layout.lgpdNotice}</p>
      )}
    </div>
  );
}
